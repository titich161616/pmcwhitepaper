...
Starting the server with various configuration options can be done using a configuration file instead of using just flags. The command used to start the server with a config file: `PMC-Chain server --config <config_file_name>`

## Export config file with default configuration

The configuration with default settings for the PMC Chain server can be exported into a config file in either `yaml` or `json` file format. This file can be used as a template for running the server using a configuration file.
...
```bash
PMC-Chain server export --type yaml
```

or just

```bash
PMC-Chain server export
```
...
```bash
PMC-Chain server export --type json
```
