...
This section aims to give a better overview of some concepts currently present in the Proof of Stake (PoS) implementation of the PMC Chain.

The PMC Chain Proof of Stake (PoS) implementation is meant to be an alternative to the existing PoA IBFT implementation, giving node operators the ability to easily choose between the two when starting a chain.

### PoS Features

The core logic behind the Proof of Stake implementation is situated within the Staking Smart Contract.

This contract is pre-deployed whenever a PoS mechanism PMC Chain chain is initialized, and is available on the address `0x0000000000000000000000000000000000001001` from block `0`.

#### Epochs

Epochs are a concept introduced with the addition of PoS to the PMC Chain.
...
**Epoch Blocks** are a concept introduced in the PoS implementation of IBFT in PMC Chain.
...
```bash
PMC-Chain genesis --epoch-size 50 ...
```

The default size of an epoch is `100000` blocks in the PMC Chain.

### Contract pre-deployment

The PMC Chain _pre-deploys_ the [Staking Smart Contract](https://github.com/0xPolygon/staking-contracts/blob/main/contracts/Staking.sol) during **genesis generation** to the address `0x0000000000000000000000000000000000001001`.
